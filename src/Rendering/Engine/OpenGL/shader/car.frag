...
layout(std140, binding = 2) uniform VehicleLightingControl{

	vec3 rightDirection;
	float useLightMask;

} uVehicleLightingControl;
...
layout(binding = 14) uniform sampler2D uTexLightMask;
...
vec3 GetEmissive()
{
	...
	{
		vec3 emissiveColor = texture(uTexEmissiveColor, fs_in.texCoord.xy).rgb * uMtl.emissiveIntensity;

		vec3 lightMask = vec3(1);
		if(uVehicleLightingControl.useLightMask!=0)
		{
			lightMask = texture(uTexLightMask, fs_in.texCoord.xy).rgb;

			vec3 headLightEmissive = vec3(0);
			float headLightMaskValue = fs_in.headlight * lightMask.r;
			headLightEmissive  = headLightMaskValue * emissiveColor;

			vec3 brakeLightEmissive = vec3(0);
			float brakeLightMaskValue = fs_in.brakeLight * lightMask.g;
			brakeLightEmissive  = brakeLightMaskValue * emissiveColor;

			vec3 turnLightEmissive = vec3(0);
			if(uVehicleLightingControl.rightDirection != vec3(0) && fs_in.turnSignal != 0)
			{
				vec3 fragDir = normalize(fs_in.localPosition);
				float isRight = dot(normalize(uVehicleLightingControl.rightDirection), fragDir);

				if((fs_in.turnSignal > 0 && isRight > 0) || (fs_in.turnSignal < 0 && isRight < 0))
				{
					float turnSignalMaskValue = abs(fs_in.turnSignal) * lightMask.b;
					turnLightEmissive = turnSignalMaskValue * emissiveColor;
				}
			}
			emissive = headLightEmissive + brakeLightEmissive + turnLightEmissive;
		}	
	}
...
vec3 Shade()
{
	...
	if(EmissiveColor.r + EmissiveColor.g + EmissiveColor.b>0.05)	
		color = color * (1 - uMtl.emissiveIntensity) + EmissiveColor;
