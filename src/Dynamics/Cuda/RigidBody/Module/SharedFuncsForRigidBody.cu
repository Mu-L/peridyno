	...
	*/
	...
	)
	{
		...
		if(abs(normal1.x) >= 0.57735) {
			u1 = Coord(normal1.y, -normal1.x, 0);
		}
		else {
			u1 = Coord(0, normal1.z, -normal1.y);
		}
		...
		constraints[baseIndex + 1].isValid = constraints[baseIndex].isValid;
		...
		constraints[baseIndex + 2].isValid = constraints[baseIndex].isValid;
	}
