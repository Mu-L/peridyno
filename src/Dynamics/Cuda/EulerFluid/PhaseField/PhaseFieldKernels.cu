...
// 			}
		...
		void PhaseFieldKernels::InterpolateVelocity(Grid3f vel, Grid1f vel_u, Grid1f vel_v, Grid1f vel_w)
		{
...
//			K_SetVelocityBoundary << < gridDims, blockDims >> >(vel_u, vel_v, vel_w);
		...
		void PhaseFieldKernels::InterpolateVelocity(Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, Grid3f vel)
		{
		...
		void PhaseFieldKernels::AdvectBackward(Grid1f dst, Grid1f src, Grid3f vel, float dt)
		{
			dim3 gridDims, blockDims;
		...
		void PhaseFieldKernels::AdvectStaggeredBackward(Grid1f dst, Grid1f src, Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, float dt, int axis)
		{
			dim3 gridDims, blockDims;
			uint3 fDims = make_uint3(src.nx(), src.ny(), src.nz());
			computeGridSize3D(fDims, make_uint3(32, 8, 1), gridDims, blockDims);
...
// 			d(ix + 1, iy + 1, iz) += val*w110;
		...
		void PhaseFieldKernels::AdvectForward(Grid1f dst, Grid1f src, Grid3f vel, float dt)
		{
			dst.reset();
...
//			LAUNCH_KERNEL(K_ClampVelocity, fDims, make_uint3(32, 8, 1), vel);
		...
		void PhaseFieldKernels::AdvectForward(Grid4f dst, Grid4f src, Grid3f vel, Grid1f weight, float dt)
		{
			dst.reset();
			weight.reset();
		...
		void PhaseFieldKernels::AdvectBackward(Grid3f dst, Grid3f src, Grid3f vel, float dt)
		{
			dim3 gridDims, blockDims;
			uint3 fDims = make_uint3(src.nx(), src.ny(), src.nz());
			computeGridSize3D(fDims, make_uint3(32, 8, 1), gridDims, blockDims);
...
// 					dst[k0] -= weight*dc;
		...
		void PhaseFieldKernels::Sharpening(Grid1f dst, Grid3f dir, Grid1f src, Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, Grid1f omega, float gamma,
			float h, float dt)
		{
			dst.reset();
...
// 			float c2 = (1.0f - c1) / 6.0f;
		...
		void PhaseFieldKernels::Jacobi(Grid1f dst, Grid1f src, Grid1f buf, Grid3f vel, float a, float c, int iteration)
		{
			dim3 gridDims, blockDims;
			uint3 fDims = make_uint3(src.nx(), src.ny(), src.nz());
			computeGridSize3D(fDims, make_uint3(32, 8, 1), gridDims, blockDims);
...
//			}
		...
		void PhaseFieldKernels::ApplyViscosity(Grid3f dst, Grid3f src, Grid3f buf, Grid1f mass, float a, int iteration)
		{
			dim3 gridDims, blockDims;
			uint3 fDims = make_uint3(src.nx(), src.ny(), src.nz());
			computeGridSize3D(fDims, make_uint3(32, 8, 1), gridDims, blockDims);
...
// 			}
		...
		void PhaseFieldKernels::PrepareForProjection(Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, GridCoef coefMatrix, Grid1f RHS, Grid1f mass, float h, float dt)
		{
			dim3 gridDims, blockDims;
...
// 				__syncthreads();
		...
		void PhaseFieldKernels::Projection(Grid1f pressure, Grid1f buf, GridCoef coefMatrix, Grid1f RHS, int numIter)
		{
			dim3 gridDims, blockDims;
		...
		void PhaseFieldKernels::UpdateVelocity(Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, Grid1f pressure, Grid1f mass, float h, float dt)
		{
			dim3 gridDims, blockDims;
			uint3 fDims = make_uint3(mass.nx(), mass.ny(), mass.nz());
			computeGridSize3D(fDims, make_uint3(32, 8, 1), gridDims, blockDims);
		...
		void PhaseFieldKernels::ApplyGravity(Grid3f v, Vec3f g, float dt)
		{
			dim3 gridDims, blockDims;
		...
		void PhaseFieldKernels::SetU(Grid1f vel_u)
		{
			dim3 gridDims, blockDims;
		...
		void PhaseFieldKernels::SetV(Grid1f vel_v)
		{
			dim3 gridDims, blockDims;
		...
		void PhaseFieldKernels::SetW(Grid1f vel_w)
		{
			dim3 gridDims, blockDims;
